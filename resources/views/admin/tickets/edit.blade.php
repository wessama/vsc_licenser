    ...
    <div class="card-body">
        <form action="{{ route("admin.tickets.update", [$ticket->id]) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group {{ $errors->has('repo') ? 'has-error' : '' }}">
                <label for="content">{{ trans('cruds.ticket.fields.repo') }}</label>
                <input id="repo" name="repo" class="form-control " value="{{ old('repo', isset($ticket) ? $ticket->repo : '') }}">
                @if($errors->has('repo'))
                ...
                <p class="helper-block">
                    {{ trans('cruds.ticket.fields.repo_helper') }}
                </p>
            </div>
